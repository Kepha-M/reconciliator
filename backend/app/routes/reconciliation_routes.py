...
from app.models import BankUpload
...
# ------------------------------------------------------
# Endpoint: Upload Bank File (Auto-Triggers Reconciliation)
# ------------------------------------------------------
@router.post("/upload-bank-file")
async def upload_bank_file(bank_file: UploadFile = File(...), db: Session = Depends(get_db)):
    """
    Uploads a bank file (CSV/XLSX), parses it, stores it in staging,
    and immediately runs reconciliation.
    """
    ...
    try:
        ...
        df = pd.read_csv(BytesIO(content)) if bank_file.filename.endswith(".csv") else pd.read_excel(BytesIO(content))
        ...
        # ✅ Immediately run reconciliation after upload
...
# ------------------------------------------------------
# Endpoint: Run Reconciliation (Reusable Function)
# ------------------------------------------------------
@router.post("/run/{upload_id}")
def run_reconciliation(upload_id: UUID, db: Session = Depends(get_db)):
    """
    Performs reconciliation between uploaded bank transactions and
    master ERP records using fuzzy string and numeric matching.
    """

    # Fetch uploaded bank records
    ...
    results = []
    match_threshold = 85  # fuzzy matching threshold
    ...
    for bank_txn in bank_records:
        ...
        for master_txn in master_records:
            ...
            amount_match = abs(bank_txn.amount - master_txn.amount) < 1.0  # tolerance
            date_match = abs((bank_txn.date - master_txn.date).days) <= 2  # ±2 days window
